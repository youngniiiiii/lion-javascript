...
console.log('max:', max);
// 여러 수 중, 최솟값
let min = Math.min(-1, 0, 1, 2, 3, 4, 5, 6);
console.log('min:', min);
// 거듭제곱
let pow = Math.pow(2, 53); // 2**53 같다  //2*53-1 가장안전한정수 이거보다큰건 bigInt써야해
console.log('pow:', pow);
...
let getRandomMinMax = (min, max) => {
  if (min > max) throw new Error('에러');
  return Math.floor(Math.random() * (max - min) + min);
};

console.log(getRandomMinMax(5, 10)); //5~10사이 난수를 반환
/* 진법 ------------------------------------------------------------------ */
